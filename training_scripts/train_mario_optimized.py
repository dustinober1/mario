#!/usr/bin/env python3
"""
Optimized Mario RL Training Script for M1/M2 Macs and GPU Acceleration
...
Usage:
    python train_mario_optimized.py
...
print("🚀 Mario RL Training - GPU Optimized")
print("=" * 40)
...
class OptimizedFrameStack(gym.Wrapper):
    ...
    def __init__(self, env, num_stack=4, resize_shape=(84, 84)):
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=255,
            shape=(resize_shape[0], resize_shape[1], num_stack),
            dtype=np.uint8
        )
...
def create_optimized_mario_env():
    ...
    env = GymToGymnasiumWrapper(env)     # Compatibility wrapper
...
def create_parallel_envs(n_envs=4):
    """Create multiple parallel environments for faster data collection."""
    if n_envs == 1:
        return DummyVecEnv([lambda: Monitor(create_optimized_mario_env())])
    else:
        return SubprocVecEnv([
            lambda: Monitor(create_optimized_mario_env()) 
...
def main():
    ...
    try:
        ...
        # Determine number of parallel environments based on device
        if device.type in ["mps", "cuda"]:
            n_envs = 8  # More environments for GPU
            timesteps = 100000  # More training steps
        else:
            n_envs = 4  # Fewer environments for CPU
            timesteps = 50000
        
        env = create_parallel_envs(n_envs)
        ...
        print(f"   📈 Speedup: ~{steps_per_second/37:.1f}x faster than CPU baseline")
        ...
        for episode in range(5):
            ...
            for _ in range(2000):
                ...
                # Track best x position across all parallel envs
        ...
        print(f"   - {n_envs}x parallel data collection")
        print(f"   - 4x frame skipping speedup")
        print(f"   - 3x memory reduction (grayscale)")
        print(f"   - Optimized batch processing")
        print(f"   - {device_name} acceleration")
